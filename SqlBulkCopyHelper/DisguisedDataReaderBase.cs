...
/// <summary>
/// Everything a DisguisedDataReader needs except how to move to the next row,
/// which is done synchronously for IEnumerable and asynchronously for IAsyncEnumerable.
/// </summary>
internal abstract class DisguisedDataReaderBase<TEntity> : DbDataReader
{
    ...
    /// <summary>
    /// The entity of the current row.
    /// </summary>
    protected abstract TEntity Current { get; }

    /// <summary>
    /// Should be called by Read/ReadAsync after moving to the next row.
    /// </summary>
    protected void SetCurrentRow(bool hasRow)
    {
        _hasCurrentRow = hasRow;
        if (hasRow)
        {
    ...
    public override object GetValue(int ordinal)
    {
        ...
        {
            _values[ordinal] = _columnDefinitions[ordinal].PropertyGetter(Current) ?? DBNull.Value;
